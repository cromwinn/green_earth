...
def one_train_test(df):
    ...
    print("MSE is : " + str(round(mean_squared_error(y_test, X_test_predicted), 3)))
...
def KFolds_plot(df, n_splits=10):
    ...
    for (i, (train_index,test_index)) in enumerate(kf.split(X)):
        ...
        ax[i].scatter(X_test['doc'], y_test_predicted, color='orange', alpha=.1, label='predicted')
...
def see_coefs(df, model):
    from operator import itemgetter

    column_list = list(df.columns)
    column_list.remove('temperature')
    column_list.remove('timestamp')

    ## "normalized"
    column_coefs = (model.coef_) * (np.array(np.array(df[column_list].median(axis=0))))

    out = []
    for i in range(len(column_list)):
        out.append([column_list[i],column_coefs[i]])
    out = sorted(out, key=itemgetter(1))
    for i in range(len(out)):
        print(f'For feature {out[-i-1][0]}, our beta (median value * beta coefficient) is {out[-i-1][1]}')
